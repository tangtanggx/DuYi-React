...
2. 几乎所有的元素的事件处理，均在document的事件中处理
   1. 一些不冒泡的事件，是直接在元素上监听，如focus事件
   2. 一些document上面没有的事件，直接在元素上监听，如播放器的播放事件
...
4. React的事件参数，并非真实的DOM事件参数，是React合成的一个对象，该对象类似于真实DOM的事件参数
   ...
   3. 为了提高执行效率，React使用事件对象池来处理事件对象，就是重用同一个事件对象
...
5. 在事件处理程序中，不要异步的使用事件对象，如果一定要使用，需要调用persist函数（他会将事件对象保存下来，不会置为null）